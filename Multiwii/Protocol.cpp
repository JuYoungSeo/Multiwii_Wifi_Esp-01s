...
int rpmPin[4] = {A0, A1, A2, A3};
uint16_t rpm_cnt[4] = {0,0,0,0};
uint8_t rpm[4] = {0,0,0,0};
uint8_t prevState[4] = {0,0,0,0};
uint8_t tof_sensor = 0;
static uint8_t CURRENTPORT=0;
int16_t gyroX, gyroY, gyroZ; 
int prevValue[4]={0,0,0,0};
String string_data[32]="";
...
static char json[20] = "{\"IotName\":\"DRONE\",\"";
...
static void s_struct_partial_data(uint8_t *cb,uint8_t siz) {
  
  char data_a[4] ={0,0,0,0};
  String ddd="";
  uint8_t len=0;
  while(siz--) {
  ddd=String(*cb++);
...
uint8_t makeWaves(int a){
  ...
  int distance = ((int)(340 * duration) / 10000) / 2;  
  return distance;
}



void makeJsonData(char* data){
  s_struct_partial((uint8_t*)&json, 20);
  s_struct_partial((uint8_t*)data, strlen(data));
  //": "
  serialize8(0x22);
  serialize8(0x3a);
  serialize8(0x22);
...
void sendToWifi(){
  ...
  //MSP_RAW_IMU
      ...
      #endif
  s_struct_partial_data((uint8_t*)&imu,12);
  
  //MSP_MOTOR
  //s_struct_partial_data((uint8_t*)&motor,8);
/*
 struct {
        uint16_t x;
        uint16_t y;
  ...
  wave_data.x = (int)makeWaves(0)*10;
  wave_data.y = (int)makeWaves(1)*10;
  s_struct_partial_data((uint8_t*)&wave_data,4);
  */
  //s_struct_partial_data((uint8_t*)&imu,12);
  sendJsonData();
  }
